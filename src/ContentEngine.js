...
window.addEventListener('message', function(e) {
	...
	if (JSON.parse(e.data).action === "getObjectTwo") {
		// if the request from the inject scripts is "getObjectTwo",
		// retrieve data from the localStorage
		getStoredObject(function(data) {
			var pass_data = {
				'action' : "getStoredTwo",
	...
	if (JSON.parse(e.data).action === "getHTML") {
		// if the request from the inject scripts is "getHTML",
		// load the HTML page
		chrome.extension.sendMessage({
			"type" : "getExternal",
			"value" : JSON.parse(e.data)
	...
	if (JSON.parse(e.data).action === "getHTMLTwo") {
		// if the request from the inject scripts is "getHTMLTwo",
		// load External pages (HTML, JSON...)
		chrome.extension.sendMessage({
			"type" : "getExternal",
			"value" : JSON.parse(e.data)
		}, function(data) {
